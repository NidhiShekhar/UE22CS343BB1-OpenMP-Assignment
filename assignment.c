                ...
                switch ( msg.type ) {
                    case READ_REQUEST:
                        ...
                        if( node.directory[memBlockAddr].state == U || node.directory[memBlockAddr].state == S) {
                            // logic for S and U states are similar hence we combine it in one if condition
                            msgReply.type = REPLY_RD;
                            msgReply.sender = threadId; //using threadID  because home node is sending this reply
                            msgReply.address = msg.address;
                            msgReply.value = node.memory[memBlockAddr]; // sends current memory value
                            sendMessage(msg.sender, msgReply);

                            //updating the directory state to shared and adding the sender to the bit vector
                    ...
                    case REPLY_RD:
                        ...
                        // we first check if theres a need to handle cache replacement
                    ...
                    case REPLY_ID:
                        ...
                        //checking if we need to handle cache replacement
                        if (node.cache[cacheIndex].state != INVALID && node.cache[cacheIndex].address != msg.address) {
                            handleCacheReplacement(threadId, node.cache[cacheIndex]);
                        }
                    ...
                    node.cache[cacheIndex].address = msg.address;
                    if (instr.type == 'W') {
                        // For write operations, use the value from the instruction
                        node.cache[cacheIndex].value = instr.value;
                    } else {
                        // For read operations that resulted in an upgrade, use the received value
                        node.cache[cacheIndex].value = msg.value;
                    }
                        ...
                        // change from SHARED to EXCLUSIVE using EVICT_SHARED
                            ...
                            if (threadId == procNodeAddr) {
                                ...
                                if (node.directory[memBlockAddr].bitVector == 0) {
                                    ...
                                    // Only one sharer remains - change directory state to EM
                                    node.directory[memBlockAddr].state = EM;

                                    // Notify the remaining sharer to change from SHARED to EXCLUSIVE
                                    for (int i = 0; i < NUM_PROCS; i++) {
                                        if (node.directory[memBlockAddr].bitVector & (1 << i)) {
                                       ...
                                       node.cache[cacheIndex].address == msg.address) {
                                // Executes on remaining sharer - update state to EXCLUSIVE
...
void sendMessage(int receiver, message msg) {
    // Acquire lock for the receiver's buffer to ensure thread safety
...
void handleCacheReplacement(int sender, cacheLine oldCacheLine) {
    // Skip if the cache line is invalid or contains invalid address
    if (oldCacheLine.state == INVALID || oldCacheLine.address == 0xFF) {
        return;
    }

    // Extract the home node's address and memory block index
    byte procNodeAddr = (oldCacheLine.address >> 4) & 0x0F;  // MSB 4 bits
    byte memBlockAddr = oldCacheLine.address & 0x0F;        // LSB 4 bits
    ...
    case EXCLUSIVE:
    case SHARED:
        // Inform home node about eviction of clean cache line
        msg.type = EVICT_SHARED;
        sendMessage(procNodeAddr, msg);
        break;

    case MODIFIED:
        // Inform home node about eviction of dirty cache line
            msg.type = EVICT_MODIFIED;
        msg.value = oldCacheLine.value;  // Send the modified value
        sendMessage(procNodeAddr, msg);
        break;
